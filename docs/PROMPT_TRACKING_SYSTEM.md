...
### Tracked Files
All these files are automatically tracked (paths relative to repo root):
- `web/app.js`
- `web/index.html`
- `server.py`
- `web/styles.css`
...
### View Prompt History
```bash
python tools/prompt_tracker.py list
```
...
```bash
python tools/prompt_tracker.py revert <number>
```

**Example:**
```bash
python tools/prompt_tracker.py revert 3
```
...
3. **You:** "Revert to before prompt #2"
   - I run: `python tools/prompt_tracker.py revert 2`
...
- ✅ **History stored in** `data/prompt_history.json`

## Files Created

- `tools/prompt_tracker.py` - Main tracking script
- `data/prompt_history.json` - History storage (auto-managed)
- `docs/PROMPT_HISTORY_README.md` - Detailed documentation

## Next Steps

**From now on**, every prompt you give me will be automatically tracked. Just use:
- `python tools/prompt_tracker.py list` to see history
- `python tools/prompt_tracker.py revert <number>` to revert
