...
2. **Corrupted file:** Restore `tools/josm-helper.py` from GitHub or from `archive/josm-helper.py.backup`
...
2. **Antivirus blocking**
   - Solution: Add exception for Python and `tools/josm-helper.py`
...
2. **Run:**
   ...
   ```cmd
   python\python.exe tools\josm-helper.py
   ```
...
When sharing `START-OSMAGIC.bat`, include the **whole project** (or at least):

**Required:**
- ✅ `START-OSMAGIC.bat` (the launcher)
- ✅ `tools/josm-helper.py` (the helper script)

**Optional:**
- `archive/josm-helper.py.backup` (backup copy)
- `python\` folder (portable Python, if used)

**NOT needed for basic online use:**
...
- [ ] `tools/josm-helper.py` exists (repository layout: see [PROJECT_LAYOUT.md](PROJECT_LAYOUT.md))
