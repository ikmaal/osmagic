...
class StorageManager {
    ...
    /** Save GeoJSON for a specific import batch (key = batch id). */
    async saveGeoJSONForBatch(batchId, geojsonData) {
        if (!this.db) await this.init();
    ...
    async loadGeoJSONForBatch(batchId) {
        if (!this.db) await this.init();
    ...
    async deleteGeoJSONForBatch(batchId) {
        if (!this.db) await this.init();
