class TaskManager {
    /** Fixed list of reviewers for attribution when working sequences */
    static REVIEWERS = Object.freeze(['ikmal.muhammad', 'ftt.jun.ang']);

    constructor() {
        this.geojsonData = null;
        this.sequences = [];
        /** Each import file is one batch: { id, label, createdAt, geojson, sequences, currentIndex, currentView } */
        this.importBatches = [];
        ...
        this.allSequencesLayout = localStorage.getItem('osmagic-all-seq-layout') === 'table' ? 'table' : 'grid'; // 'grid' | 'table'
    ...
    async handleFileUpload(event) {
        ...
        for (const result of results) {
            if (result.status !== 'fulfilled') {
                ...
                errors.push(`${r.fileName}: ${r.error.message}`);
                continue;
            }
            const { geojson, fileName } = result.value;
            const fc = this.normalizeToFeatureCollection(geojson);
            if (!fc.features || fc.features.length === 0) {
                errors.push(`${fileName}: No features found`);
                continue;
            }

            const batchId = this.newImportBatchId();
            const label = fileName;
            let sequences;
            try {
                if (fc.features.length > 50) {
                    if (fileInfo) {
                        fileInfo.textContent = `Processing ${fileName}: ${fc.features.length} features...`;
                    }
                    sequences = await this.buildSequencesFromGeoJSONAsync(fc, new Map(), (progress) => {
                        if (fileInfo) {
                            fileInfo.textContent = `${fileName}: ${progress}%...`;
                        }
                    });
                } else {
                    sequences = this.buildSequencesFromGeoJSONSync(fc, new Map());
                }
            } catch (err) {
        ...
        this.activeBatchId = newBatchIds[newBatchIds.length - 1];
        this.applyBatchToWorkspace(this.activeBatchId);

        try {
            await this.saveToStorage();
        } catch (error) {
            console.error('Error saving after import:', error);
        }

        const errorMsg = errors.length > 0 ? ` (${errors.length} error(s))` : '';
        if (fileInfo) {
            fileInfo.textContent = `✓ ${created} import batch(es) added${errorMsg}. Active: ${this.importBatches.find(b => b.id === this.activeBatchId)?.label || ''}`;
        }
    ...
    commitActiveBatch() {
        ...
        const batch = this.importBatches.find(b => b.id === this.activeBatchId);
        if (!batch) return;
        ...
        batch.geojson = this.geojsonData ? JSON.parse(JSON.stringify(this.geojsonData)) : null;
        batch.sequences = JSON.parse(JSON.stringify(this.sequences));
        batch.currentIndex = this.currentIndex;
        batch.currentView = this.currentView;
    }

    applyBatchToWorkspace(batchId) {
        const batch = this.importBatches.find(b => b.id === batchId);
        if (!batch) return;
        this.activeBatchId = batchId;
        this.geojsonData = batch.geojson ? JSON.parse(JSON.stringify(batch.geojson)) : null;
        this.sequences = JSON.parse(JSON.stringify(batch.sequences || []));
        this.sequences.forEach(seq => {
            seq.reviewedBy = this.migrateReviewerName(seq.reviewedBy);
        });
        this.currentView = batch.currentView || 'all';
        const maxIdx = Math.max(0, this.sequences.length - 1);
        this.currentIndex = Math.min(Math.max(0, batch.currentIndex ?? 0), maxIdx);
    ...
    onBatchSelectChange(batchId) {
        if (!batchId || batchId === this.activeBatchId) return;
        this.commitActiveBatch();
        this.applyBatchToWorkspace(batchId);
    ...
    renderBatchesUI() {
        ...
        wrap.style.display = '';
        sel.innerHTML = '';
        this.importBatches.forEach((b, i) => {
            const opt = document.createElement('option');
            opt.value = b.id;
            opt.textContent = `${i + 1}. ${b.label}`;
            if (b.id === this.activeBatchId) {
                opt.selected = true;
            }
            sel.appendChild(opt);
        });
        sel.onchange = (e) => this.onBatchSelectChange(e.target.value);
        if (nameInput) {
            nameInput.disabled = false;
        }
        this.syncBatchNameField();
    }

    syncBatchNameField() {
        const input = document.getElementById('batchNameInput');
        if (!input) return;
        const batch = this.activeBatchId
            ? this.importBatches.find(b => b.id === this.activeBatchId)
            : null;
        if (!batch) {
            input.value = '';
            input.disabled = true;
    ...
    applyBatchRename() {
        ...
        if (!name) {
            alert('Enter a name for this batch.');
            this.syncBatchNameField();
            return;
        }
        batch.label = name;
    ...
    async deleteActiveBatch() {
        if (!this.activeBatchId || this.importBatches.length === 0) return;
        const batch = this.importBatches.find(b => b.id === this.activeBatchId);
        const label = batch?.label || 'this batch';
        if (!confirm(`Delete import batch "${label}"?\n\nThis removes stored data for that batch only (other batches stay).`)) {
            return;
        }

        this.commitActiveBatch();
        const idToRemove = this.activeBatchId;
        const idx = this.importBatches.findIndex(b => b.id === idToRemove);
        if (idx === -1) return;

        this.importBatches.splice(idx, 1);

        try {
            await storageManager.deleteGeoJSONForBatch(idToRemove);
        } catch (err) {
            console.warn('Could not remove batch GeoJSON from storage:', err);
        }
        ...
        if (this.importBatches.length === 0) {
            ...
            this.syncViewChrome('all');
        } else {
            const newIdx = Math.min(idx, this.importBatches.length - 1);
            this.activeBatchId = this.importBatches[newIdx].id;
            this.applyBatchToWorkspace(this.activeBatchId);
        }
        ...
        const fileInfo = document.getElementById('fileInfo');
        if (fileInfo && this.importBatches.length === 0) {
            fileInfo.textContent = 'Batch deleted. Import a file to start again.';
        }
    }

    featureSequenceId(feature, index) {
        return String(
    ...
    migrateReviewerName(name) {
        if (name === 'ikma.muhammad') return 'ikmal.muhammad';
        return name !== undefined && name !== null ? String(name) : '';
    }

    /** @returns {{ status: string, reviewedBy: string }} */
    normalizePreservedEntry(entry) {
        if (entry == null) return { status: '', reviewedBy: '' };
        if (typeof entry === 'string') return { status: entry, reviewedBy: '' };
        return {
            status: entry.status !== undefined ? entry.status : '',
            reviewedBy: this.migrateReviewerName(entry.reviewedBy)
        };
    }

    buildPreservedSequenceMeta(sequences) {
        const m = new Map();
        if (!sequences) return m;
    ...
    reviewerOptionsInnerHtml(selected) {
        const v = selected || '';
        let html = '<option value="">—</option>';
        for (const name of TaskManager.REVIEWERS) {
            html += `<option value="${this.escapeHtmlAttr(name)}"${v === name ? ' selected' : ''}>${this.escapeHtml(name)}</option>`;
        }
        return html;
    }

    fillReviewerSelectElement(selectEl, currentValue) {
        if (!selectEl) return;
        selectEl.innerHTML = this.reviewerOptionsInnerHtml(currentValue);
    }

    buildSequencesFromGeoJSONSync(geojson, preservedById = new Map()) {
        if (!geojson.features || !Array.isArray(geojson.features)) {
            throw new Error('Invalid GeoJSON: missing features array');
        }
        const sequenceMap = new Map();
        geojson.features.forEach((feature, index) => {
            const sequenceId = this.featureSequenceId(feature, index);
            if (!sequenceMap.has(sequenceId)) {
                const preserved = this.normalizePreservedEntry(preservedById.get(sequenceId));
                sequenceMap.set(sequenceId, {
                    id: sequenceId,
                    features: [],
                    status: preserved.status,
                    reviewedBy: preserved.reviewedBy,
    ...
    async buildSequencesFromGeoJSONAsync(geojson, preservedById = new Map(), progressCallback) {
        if (!geojson.features || !Array.isArray(geojson.features)) {
            throw new Error('Invalid GeoJSON: missing features array');
        }
        ...
        for (let start = 0; start < totalFeatures; start += chunkSize) {
            ...
            for (let i = start; i < end; i++) {
                ...
                const sequenceId = this.featureSequenceId(feature, i);
                if (!sequenceMap.has(sequenceId)) {
                    const preserved = this.normalizePreservedEntry(preservedById.get(sequenceId));
                    sequenceMap.set(sequenceId, {
                        id: sequenceId,
            ...
            if (progressCallback) {
                progressCallback(Math.round((end / totalFeatures) * 50));
            }
        ...
        for (let start = 0; start < sequences.length; start += statsChunkSize) {
            ...
            if (progressCallback) {
                progressCallback(50 + Math.round((end / sequences.length) * 50));
            }
            await new Promise(resolve => setTimeout(resolve, 0));
        }
    ...
    async processGeoJSON(geojson) {
        ...
        const preserved = this.buildPreservedSequenceMeta(this.sequences);

        this.sequences = this.buildSequencesFromGeoJSONSync(geojson, preserved);
    ...
    async processGeoJSONAsync(geojson, progressCallback) {
        ...
        this.sequences = await this.buildSequencesFromGeoJSONAsync(geojson, preserved, progressCallback);
        this.geojsonData = geojson;
    ...
    syncViewChrome(view) {
        ...
        if (titleEl) {
            ...
            titleEl.textContent = titles[view] || 'Sequences';
        }
    }
    ...
    setAllSequencesLayout(layout) {
        if (layout !== 'grid' && layout !== 'table') return;
        this.allSequencesLayout = layout;
        try {
            localStorage.setItem('osmagic-all-seq-layout', layout);
        } catch (_) { /* ignore */ }
        this.renderAllTasksView();
    }

    buildAllSequencesTable(sequences) {
        const rows = sequences.map(seq => {
            ...
            const stats = seq.features && seq.features.length
                ...
                : {
                    features: seq.featureCount || 0,
                    nodes: seq.nodeCount || 0,
                    ways: seq.wayCount || 0
                };
            return `
                <tr class="sequences-table-row${this.sequenceDoneClass(seq)}">
                    ...
                    <td>
                        <select class="status-dropdown-inline sequences-table-status" data-sequence-id="${idAttr}" onchange="taskManager.updateStatus('${escapedId}', this.value)">
                    ...
                    <td class="sequences-td-reviewer">
    ...
    renderAllTasksView() {
        ...
        const layout = this.allSequencesLayout;

        // Render grid of all sequence IDs (clickable) or table
        ...
        this.setTaskDisplayContent(`
            <div class="all-tasks-list">
                <div class="all-tasks-toolbar">
                    ...
                    <div class="all-tasks-toolbar-tools">
                        <div class="all-tasks-search-wrap">
                            <input 
                                type="text" 
                        ...
                        <div class="sequences-view-toggle" role="group" aria-label="Sequence list layout">
                            <span class="sequences-view-toggle-label">Layout</span>
                            <button type="button" class="layout-toggle ${layout === 'grid' ? 'active' : ''}" onclick="taskManager.setAllSequencesLayout('grid')">Grid</button>
                            <button type="button" class="layout-toggle ${layout === 'table' ? 'active' : ''}" onclick="taskManager.setAllSequencesLayout('table')">Table</button>
                        </div>
                        <button type="button" class="btn btn-primary btn-export-csv" onclick="taskManager.exportAllToCSV()">
                            Export CSV
                        </button>
                    </div>
                </div>
    ...
    renderDetailedView() {
        ...
        this.setTaskDisplayContent(`
            ...
            <div class="task-card">
                ...
                <div class="status-section">
                    ...
                    <select id="statusDropdown" class="status-dropdown" data-sequence-id="${seqIdAttr}">
                ...
                <div class="status-section">
                    <label class="status-label" for="reviewerDropdown">Reviewer</label>
                    <select id="reviewerDropdown" class="status-dropdown" data-sequence-id="${seqIdAttr}">${this.reviewerOptionsInnerHtml(displaySequence.reviewedBy)}</select>
                </div>
        ...
        if (reviewerDropdown) {
            reviewerDropdown.addEventListener('change', (e) => {
                this.updateSequenceReviewer(displaySequence.id, e.target.value);
            });
        }
    }
    ...
    async updateSequenceReviewer(sequenceId, reviewer) {
        const sequence = this.sequences.find(s => String(s.id) === String(sequenceId));
        if (!sequence) return;
        const v = reviewer || '';
        if (v !== '' && !TaskManager.REVIEWERS.includes(v)) return;
        sequence.reviewedBy = v;
        await this.saveToStorage();
        const modalSel = document.getElementById('previewReviewerSelect');
        if (modalSel && this.currentPreviewSequence && String(this.currentPreviewSequence.id) === String(sequenceId)) {
            modalSel.value = v;
        }
        this.renderCurrentTask();
    }
    ...
    /**
     * Approximate center of a sequence (centroid of axis-aligned bbox of all feature coordinates).
     * @returns {{ lat: number, lon: number } | null}
     */
    getSequenceCenterLatLon(sequence) {
        if (!sequence?.features?.length) return null;
        let minLat = Infinity;
        let maxLat = -Infinity;
        let minLon = Infinity;
        let maxLon = -Infinity;
    ...
    formatSequenceLatLon(sequence) {
        const c = this.getSequenceCenterLatLon(sequence);
        if (!c) return '—';
    ...
    /**
     * Single-line coordinates UI: copyable button, or em dash when unknown.
     */
    coordinatesCopyHtml(ll, extraClass = '') {
        if (ll === '—') {
            return `<span class="sequence-latlon sequence-latlon--empty ${extraClass}">—</span>`;
        }
        const safe = this.escapeHtmlAttr(ll);
        return `<button type="button" class="sequence-latlon sequence-latlon--copy ${extraClass}" data-copy="${safe}" onclick="taskManager.onCoordinatesCopyClick(event, this)" title="Click to copy to clipboard">${ll}</button>`;
    }

    async onCoordinatesCopyClick(event, el) {
        if (event) {
            event.stopPropagation();
            event.preventDefault();
        }
        const text = el?.dataset?.copy || el?.getAttribute('data-copy');
        if (!text || text === '—') return;
        try {
            ...
            el.classList.add('coords-copied');
            setTimeout(() => el.classList.remove('coords-copied'), 900);
            const fileInfo = document.getElementById('fileInfo');
            if (fileInfo) {
    ...
    async previewSequence(sequenceId) {
        ...
        if (previewReviewerSelect) {
            this.fillReviewerSelectElement(previewReviewerSelect, sequence.reviewedBy || '');
            previewReviewerSelect.onchange = (e) => {
                this.updateSequenceReviewer(sequence.id, e.target.value);
            };
        }
    ...
    async saveToStorage(options = {}) {
        try {
            ...
            const batchesMeta = this.importBatches.map(b => ({
                id: b.id,
                label: b.label,
                createdAt: b.createdAt,
                currentIndex: b.currentIndex,
                currentView: b.currentView || 'all',
    ...
    async loadFromStorage() {
        try {
            ...
            if (taskData.batches && Array.isArray(taskData.batches) && taskData.batches.length > 0) {
                this.importBatches = [];
                for (const meta of taskData.batches) {
                    let gj = await storageManager.loadGeoJSONForBatch(meta.id);
                    if (!gj) {
                        gj = await storageManager.loadGeoJSONData();
                    }
                    if (!gj || !gj.features) {
                        this.importBatches.push({
                            id: meta.id,
                ...
                this.activeBatchId = taskData.activeBatchId || this.importBatches[0]?.id || null;
                if (this.activeBatchId && !this.importBatches.find(b => b.id === this.activeBatchId)) {
                    this.activeBatchId = this.importBatches[0]?.id || null;
                }
                if (this.activeBatchId) {
            ...
            // Legacy: single combined dataset under geojsonData "main"
            this.importBatches = [];
            this.activeBatchId = null;
            ...
            if (taskData.sequences && Array.isArray(taskData.sequences)) {
                if (this.geojsonData && this.geojsonData.features) {
                    ...
                    this.geojsonData.features.forEach((feature, idx) => {
                        const sequenceId = this.featureSequenceId(feature, idx);
                    ...
                    taskData.sequences.forEach(seq => {
                        savedMetaMap.set(String(seq.id), {
                            status: seq.status,
                            reviewedBy: seq.reviewedBy !== undefined ? seq.reviewedBy : ''
                        });
                    });

                    this.sequences = Array.from(sequenceMap.values()).map(seq => {
                        const meta = savedMetaMap.get(String(seq.id)) || {};
                        const stats = this.calculateStats(seq.features);
                        return {
                            ...seq,
                            status: meta.status !== undefined ? meta.status : '',
                            reviewedBy: meta.reviewedBy !== undefined ? meta.reviewedBy : '',
                            featureCount: stats.features,
                            nodeCount: stats.nodes,
                            wayCount: stats.ways,
                            date: new Date().toLocaleDateString()
                ...
                this.activeBatchId = legacyId;
                if (this.geojsonData) {
                    await storageManager.saveGeoJSONForBatch(legacyId, this.geojsonData);
                }
                await this.saveToStorage();
