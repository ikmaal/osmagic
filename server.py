#!/usr/bin/env python3
"""
Local HTTP server for OSMAGIC (serves the `web/` folder).
Run from the repository root, then open http://localhost:8000
"""
...
def main():
    if not os.path.isdir(WEB_DIR):
        print(f"ERROR: Web root not found: {WEB_DIR}")
        return
    ...
    with socketserver.TCPServer(("", PORT), Handler) as httpd:
        ...
        print("=" * 60)
        print("OSMAGIC local server is running!")
