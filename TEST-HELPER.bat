...
set "SCRIPT_DIR=%~dp0"
set "HELPER_SCRIPT=%SCRIPT_DIR%tools\josm-helper.py"
...
echo [2] Checking tools\josm-helper.py...
if not exist "%HELPER_SCRIPT%" (
    echo [!] tools\josm-helper.py not found
    pause
    exit /b 1
)
echo [OK] tools\josm-helper.py found
echo.
...
echo echo ======================================== >> "%LAUNCHER%"
echo echo. >> "%LAUNCHER%"
echo "%PYTHON_PATH%" tools\josm-helper.py >> "%LAUNCHER%"
